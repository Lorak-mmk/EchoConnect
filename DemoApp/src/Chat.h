...
public:
    Chat(std::string title) : AAction(title) {
        ...
        arguments.insert({"listen", Argument("who initiates connection (0: we connect, other side listens / any other number: we listen, other side connects)", ArgumentType::INTEGER)});
