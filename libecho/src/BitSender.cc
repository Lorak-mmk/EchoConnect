...
BitSender::BitSender(int windowSize, int lo = DEFAULT_LO_FREQ, int hi = DEFAULT_LO_FREQ)
    : ISender(windowSize), loFreq(lo), hiFreq(hi) {
    outputFormat = AudioFormatFactory::getOutputFormat();
    output = std::make_unique<AudioOutput>(outputFormat);
}

std::vector<char> BitSender::encode(const std::vector<uint8_t> &data) override {
...
std::vector<SampleType> BitSender::encode_byte(uint8_t data) override {
    std::vector<SampleType> result;
