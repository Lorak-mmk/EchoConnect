...
ViewPtr ReceiveText::runAction() {
	...
	try {
		...
		std::cout << setFormatting({ConsoleFormat::T_RED}) << " Unfortunately we could detect any transmission, press enter to return to the previous view...\n" << clearFormatting();
		Utils::waitForEnter();
	}
    ...
    try {
        ...
        std::cout << setFormatting({ConsoleFormat::T_RED}) << " Unfortunately transmission haven't finished correctly and the message isn't complete.\n" << clearFormatting();
    }
